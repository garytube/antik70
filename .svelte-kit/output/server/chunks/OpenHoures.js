...
const SITE = {
  ...
  DE: {
    ...
    HERO_SUBLINE: "Antike Möbelstücke aus den vielen Epochen erzählen Ihre Geschichte vergangener Zeiten egal ob Vertiko, Kommode, Küchenbuffet, Schrank oder Sekretäre immer mit Liebe restauriert und bereit Ihr Zuhause zu bereichern. Erleben Sie unsere Welt in der sich Geschichte und Stil vereinigen.",
    HERO_CTA: "Entdecken Sie unsere Raritäten",
    LAMP_HEADLINE: "Lampen der besonderen Art",
    LAMP_SUBLINE: "Ausgewählte und bestens restaurierte Lampen aus der Gründerzeit bis zum Jugendstil. Decken - Tisch - Wand und Stehleuchten in großer Auswahl. Dazu eine Serie von Messingleuchten, die als Besonderheit über eine stufenlose Verstellbarkeit verfügen. Diese Zuglampen - Serie ist in Ihrer Variabilität unvergleichlich und löst fast jedes Beleuchtungsproblem.",
    LAMP_CTA: "Unsere Lampen",
    MEGA_HEADLINE: "Restaurationen, Reparaturen\nEin & Umbauten an Möbeln",
    MEGA_SUBLINE: "	Wir sind Experten für Reparaturen und Restaurationen. Sie können sich darauf verlassen, dass wir Ihre Möbel in ihren ursprünglichen Glanz versetzen oder sie in etwas Neues und \nAufregendes verwandeln können.",
    MEGA_CONTACT: "Zögern Sie nicht, uns zu kontaktieren!",
    MEGA_CTA: "Unser Reparatur Service",
    NAVI_LINKS: [
      {
        label: "Lampen",
        href: "#lampen"
...
const OpenHoures = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  return `<h4 class="text-lg font-bold mb-4 row-start-1">${escape(SITE.DE.OPEN_HOURS)}</h4> <p class="mb-2 row-start-2"><span class="font-bold block">${escape(SITE.DE.OPEN_HOURS_1)}</span> ${escape(SITE.DE.OPEN_HOURS_2)}</p> <p class="row-start-2"><span class="font-bold block">${escape(SITE.DE.OPEN_HOURS_3)}</span> ${escape(SITE.DE.OPEN_HOURS_4)}</p>`;
});
export {
  OpenHoures as O,
  SITE as S
